...
import 'package:repositories/src/settings_repository.dart';
...
class FirebaseSettingsRepository implements SettingsRepository {
  ...
  Stream<UserSettings> settings() {
    ...
    DocumentReference settingsDoc = FirebaseFirestore.instance.collection("users").doc(user.uid);
  ...
  Future<void> updateSettings(UserSettings userSettings) async {
    User user = _auth.currentUser!;
    DocumentReference settingsDocument = FirebaseFirestore.instance.collection("users").doc(user.uid);
    return settingsDocument.update({"counterTeamMembers": userSettings.counterTeamMembers});
  }
